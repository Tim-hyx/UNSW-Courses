...
class SortingExperimenter(tk.Tk):
    ...
    def red_help(self):
        tkinter.messagebox.showinfo(
                ...
                'colored red when they are about to be swapped.\n\n'
...
class ListToSort(tk.Frame):
            ...
            for i in range(n, 2 * n):
                for j in range(i, self.length, n):
                    ...
                    while k >= n and self.list[k - n] > self.list[k]:
                        ...
                        yield
                        self.list[k - n], self.list[k] =\
                                            self.list[k], self.list[k - n]
                        if n != 1 or k != n or j != self.length - n:
                            self.displayed_list.itemconfig(
                                    self.cells[k - n], text=self.list[k - n],
                                    fill=self.nonmoving_item_colour,
                                    font=('normal', 0, 'normal')
                                                          )
                            self.displayed_list.itemconfig(
                                    self.cells[k], text=self.list[k],
                                    fill=self.nonmoving_item_colour,
                                    font=('normal', 0, 'normal')
                                                          )
                            yield
                            self.displayed_list.itemconfig(
                                    self.cells[k], fill=self.live_item_colour
                                                          )
                        else:
                            self.displayed_list.itemconfig(
                                    self.cells[k - n], text=self.list[k - n],
                    ...
                    if k - n >= 0:
                        ...
                        self.displayed_list.itemconfig(
                                self.cells[k], fill=self.nonmoving_item_colour,
                                font=('normal', 0, 'bold')
                                                      )
                        yield
                        self.displayed_list.itemconfig(
                                self.cells[k - n], fill=self.live_item_colour,
                                font=('normal', 0, 'normal')
                                                      )
                        self.displayed_list.itemconfig(
                                self.cells[k], fill=self.live_item_colour,
    ...
    def trace_mergesort(self, L1, L2, start, end, depth):
        ...
        while i1 < half_length and i2 < end:
            if depth % 2:
                self.displayed_list.itemconfig(
                        self.cells[i1], fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                              )
                self.displayed_list.itemconfig(
                        self.cells[i2], fill=self.nonmoving_item_colour,
                        font=('normal', 0, 'bold')
                                              )
                yield
            else:
            ...
            if L2[i1] <= L2[i2]:
                if depth % 2:
                    self.displayed_list.itemconfig(
                            self.cells[i1], fill=self.moving_item_colour,
                            font=('normal', 0, 'normal')
                                                  )
                    self.displayed_list.itemconfig(
                            self.cells[i2], fill=self.live_item_colour,
                            font=('normal', 0, 'normal')
                                                  )
                    self.displayed_extra_list.itemconfig(self.extra_cells[i],
                                                         text='*'
                    ...
                    L1[i] = L2[i1]
                    self.displayed_list.itemconfig(
                            self.cells[i1], fill=self.live_item_colour
                                                  )
                    self.displayed_extra_list.itemconfig(
                    ...
                    self.displayed_extra_list.itemconfig(
                            self.extra_cells[i1], fill=self.moving_item_colour,
                            font=('normal', 0, 'normal')
                                                        )
                    self.displayed_extra_list.itemconfig(
                            self.extra_cells[i2], fill=self.live_item_colour,
                            font=('normal', 0, 'normal')
                                                        )
                    ...
                    L1[i] = L2[i1]
                    self.displayed_extra_list.itemconfig(
                            self.extra_cells[i1], fill=self.live_item_colour
                                                        )
                    if depth:
                        self.displayed_list.itemconfig(
                                self.cells[i], text=self.list[i],
                ...
                if depth % 2:
                    ...
                    self.displayed_list.itemconfig(
                            self.cells[i2], fill=self.moving_item_colour,
                            font=('normal', 0, 'normal')
                                                  )
                    self.displayed_extra_list.itemconfig(self.extra_cells[i],
                                                         text='*'
                    ...
                    self.displayed_extra_list.itemconfig(
                            self.extra_cells[i2], fill=self.moving_item_colour,
                            font=('normal', 0, 'normal')
                                                        )
                    self.displayed_list.itemconfig(self.cells[i], text='*')
                    yield
        ...
        while i1 < half_length:
            if depth % 2:
                ...
                if depth:
                    ...
                    self.displayed_list.itemconfig(self.cells[i],
                            text=self.list[i], fill=self.in_place_item_colour
                                                  )
                yield
                if depth:
                    self.displayed_list.itemconfig(
    ...
    def trace_batcher_sort(self):
        if self.length == 2:
            if self.list[0] > self.list[1]:
                ...
                self.list[0], self.list[1] = self.list[1], self.list[0]
                self.displayed_list.itemconfig(
                        self.cells[0], text=self.list[0],
                        fill=self.in_place_item_colour,
                        font=('normal', 0, 'normal')
                                              )
                self.displayed_list.itemconfig(
                        self.cells[1], text=self.list[1],
                        fill=self.in_place_item_colour,
                        font=('normal', 0, 'normal')
                                              )
                yield
                ...
                self.displayed_list.itemconfig(
                        self.cells[1], fill=self.in_place_item_colour,
                        font=('normal', 0, 'normal')
                                              )
                yield
                return
