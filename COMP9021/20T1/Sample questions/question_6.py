...
def f(for_seed, n, upper_bound):
    ...
    all_square_number = list(chain(*square))
    duplicates = set(k for k, v in Counter(all_square_number).items() if v > 1)
    if not duplicates:
        new_square = sorted(all_square_number)
        result = []
        for i in range(0, len(new_square), n):
            result.append(new_square[i:i + n])
        ordered_square = zip(*result)
        ...
        print(f"It is not a good square because it contains duplicates, namely: {' '.join(str(e) for e in sorted(duplicates))}")
