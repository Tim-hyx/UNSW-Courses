...
W, lr, start, Iteration, num_none = [float(w) for w in input('input initial weights: ').split()], float(
    input('input learning rate: ')), ord(input('start point: ')) - 97, 0, 0
while num_none - num_examples:
    ...
    print(
        f"Iteration: {Iteration:>2}, Weights: {W}, Training Example: {Training_Example}, Xs: {X[:-1]}, Class: {'+' if X[-1] > 0 else '-'}, s: {s:>+.1f}, Action: {Action:^8}")
